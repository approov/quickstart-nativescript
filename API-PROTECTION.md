...
Note that this will use [Managed Trust Roots](https://approov.io/docs/latest/approov-usage-documentation/#managed-trust-roots) to ensure that no Man-in-the-Middle attacks on your app's communication are possible.Note that this will use [Managed Trust Roots](https://approov.io/docs/latest/approov-usage-documentation/#managed-trust-roots) to ensure that no Man-in-the-Middle attacks on your app's communication are possible.
...
.catch(error => {
    if (error.type == "rejection")
        ...
        // error.rejectionReasons may be used to present information to the user
        // (note error.rejectionReasons is only available if the feature is enabled, otherwise it is
